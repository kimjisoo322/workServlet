...
<body>
    <h2>1. 로그인 폼</h2>
    <!--  ResponseMain.jsp로 포워드.매개 변수 loginErr가 전달되고 로그인 실패 메시지를 화면에 출력
     -->
   	<%
   		String loginErr = request.getParameter("loginErr");
   		if("Y".equals(loginErr)){
   			
   		out.print("로그인에 실패했습니다. 아이디와 비밀번호를 확인해주세요.");
   		}
   	%>
    <form action="./ResponseLogin.jsp" method="post">
    <!--  유효성 검사(validation check) : 사용자의 입력값이 유효한지 체크 -->
    						<!-- forward : 요청해서 서버에서 받은 페이지를 다시 요청했기에 첫번째 요청한 것이 공유되어 
    						아이디 패스워드를 갖고 있음. 페이지를 다시 전환했을 때 아이디, 값을 출력해볼 수 있는 것
    						 -->
