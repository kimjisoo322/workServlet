...
<!--  액션태그는 태그의 형태를 지니고 있어 소스의 이질감이 줄어들고 
	  jsp코드보다 훨씬 짧은 코드로 동일한 기능을 구현할 수 있음 
...
<!-- jsp:param, jsp:forward - 쿼리스트링 방식   = getParameter -->

	<%=request.getParameter("param1") %><br>
	<%=request.getParameter("param2") %><br>
	<%=request.getParameter("param3") %><br>
	
	<!--  jsp:inclue 액션태그를 이용해 다른페이지를 불러올 때 jsp:param 액션태그를 이용시 파라미터를 전달할 수 있다.  -->
	<h2> jsp:inclue페이지에 파라메터 넘기기</h2>
